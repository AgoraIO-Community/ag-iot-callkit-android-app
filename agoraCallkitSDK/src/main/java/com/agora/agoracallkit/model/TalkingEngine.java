 ...
 */
package com.agora.agoracallkit.model;
...
public class TalkingEngine implements AGEventHandler, IVideoFrameObserver {
    ...
    ///////////////////// Constant Definition /////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////
    private final String TAG = "CALLKIT/TalkingEngine";
    ...
    private AgoraCallKit.NetworkStatus mRtcStatus = new AgoraCallKit.NetworkStatus();
    ...
    public synchronized AgoraCallKit.NetworkStatus getNetworkStatus() {
        AgoraCallKit.NetworkStatus retStatus = new AgoraCallKit.NetworkStatus();
    ...
    public synchronized boolean joinChannel(String channel, String token,
        ...
        // 加入频道
//        ChannelMediaOptions mediaOpts = new ChannelMediaOptions();
//        mediaOpts.autoSubscribeVideo = mInitParam.mSubscribeVideo;
//        mediaOpts.autoSubscribeAudio = mInitParam.mSubscribeAudio;
//        mediaOpts.publishAudioTrack = true; // mInitParam.mPublishAudio;
//        mediaOpts.publishSecondaryCameraTrack = mInitParam.mPublishVideo;
//        mediaOpts.channelProfile = Constants.CHANNEL_PROFILE_LIVE_BROADCASTING;
//        mediaOpts.clientRoleType = mRtcEngCfg.mClientRole;
//       int ret = mRtcEngine.joinChannel(token, channel, uid, mediaOpts);
    ...
    public synchronized boolean stopTalking()
    {
        ALog.getInstance().d(TAG, "<stopTalking>");
        return true;
    }
    ...
    public synchronized boolean setRemoteVideoView(SurfaceView remoteView, int remoteUid)
    {
        VideoCanvas videoCanvas = new VideoCanvas(remoteView, VideoCanvas.RENDER_MODE_FIT, remoteUid);
        int ret = mRtcEngine.setupRemoteVideo(videoCanvas);

        ALog.getInstance().d(TAG, "<setRemoteVideoView> remoteView=" + remoteView
                + ", remoteUid=" + remoteUid + ", ret=" + ret);
        return true;
    }

    public synchronized boolean setTalkingParam(AgoraCallKit.TalkingParam talkingParam) {
    ...
    public synchronized boolean setLocalVoiceType(AgoraCallKit.VoiceType voice_type) {
        int voice_changer = Constants.VOICE_CHANGER_OFF;
        switch (voice_type) {
            case OLDMAN:
                voice_changer = Constants.VOICE_CHANGER_OLDMAN;
                break;

            case BABYBOY:
                voice_changer = Constants.VOICE_CHANGER_BABYBOY;
                break;

            case BABYGIRL:
                voice_changer = Constants.VOICE_CHANGER_BABYGIRL;
                break;

            case ZHUBAJIE:
                voice_changer = Constants.VOICE_CHANGER_ZHUBAJIE;
                break;

            case ETHEREAL:
                voice_changer = Constants.VOICE_CHANGER_ETHEREAL;
                break;

            case HULK:
                voice_changer = Constants.VOICE_CHANGER_HULK;
                break;
        }

        int ret = mRtcEngine.setAudioEffectPreset(voice_changer);
        ALog.getInstance().d(TAG, "<setLocalVoiceChanger> voice_changer=" + voice_changer + ", ret=" + ret);
        return (ret == Constants.ERR_OK);
    }
